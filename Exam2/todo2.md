...
- 2026-04-28 已采用弱化表述方案，没有新增不同 `M` 的收敛测试。
- 第 4.4 节对三种方法接近性的说明已改为：在本次基准实验的 `M=252, N=100000` 设置下价格较为接近，价格差异同时受到 Monte Carlo 抽样误差和离散误差影响。
- 第 7.1 节已弱化“二元期权对离散误差更敏感”和“三种方法趋势完全一致，说明数值方案稳定可靠”的表述。
- 第 7.2 节已弱化 “`M=252` 可有效降低误差” 的表述，改为说明若要量化时间步长影响，需要额外做不同 `M` 下的收敛测试。
- 已重新运行并保存整本 `Answer.ipynb`，使代码输出与修改后的表述一致。
